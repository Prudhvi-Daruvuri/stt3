...
import sys
import os
# Force import from the exact local path to avoid conflicts with other RealtimeSTT installations
local_realtimestt_path = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', 'RealtimeSTT'))
sys.path.insert(0, os.path.dirname(local_realtimestt_path))

# Remove any existing RealtimeSTT modules from cache to force fresh import
modules_to_remove = [key for key in sys.modules.keys() if key.startswith('RealtimeSTT')]
...
def parse_arguments():
    ...
    parser.add_argument('-b', '--backend', type=str, default='faster_whisper', choices=['faster_whisper', 'parakeet'],
                        help='STT backend to use. Options: faster_whisper (default), parakeet (requires nemo_toolkit[asr]). Default is faster_whisper.')
    ...
    parser.add_argument('--batch', '--batch_size', type=int, default=16, help='Batch size for inference. This parameter controls the number of audio chunks processed in parallel during transcription. Default is 16.')

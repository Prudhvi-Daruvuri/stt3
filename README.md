# RealtimeSTT with Multi-Backend Support

*Easy-to-use, low-latency speech-to-text library for realtime applications with multiple ASR backend support*

## 🚀 **NEW: Multi-Backend Architecture**

This enhanced version of RealtimeSTT now supports **multiple ASR (Automatic Speech Recognition) backends**, allowing you to choose the best transcription engine for your specific needs:

- **faster_whisper**: The original, reliable backend (default)
- **parakeet**: NVIDIA NeMo Parakeet model for advanced transcription

### Backend Selection
```bash
# Use faster_whisper backend (default)
python stt_server.py --backend faster_whisper

# Use Parakeet backend (requires nemo_toolkit[asr])
python stt_server.py --backend parakeet
```

### Key Features
- ✅ **Modular Architecture**: Clean separation of inference logic
- ✅ **Backend Switching**: Easy selection between different ASR models
- ✅ **Dependency Management**: Graceful handling of missing dependencies
- ✅ **Backward Compatibility**: Existing code continues to work unchanged
- ✅ **Performance Isolation**: Each backend runs in its own process

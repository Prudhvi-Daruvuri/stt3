...
from .faster_whisper_transcription_worker_module import TranscriptionWorker
try:
    from .parakeet_transcription_worker_module import ParakeetTranscriptionWorker
    PARAKEET_AVAILABLE = True
except ImportError:
    PARAKEET_AVAILABLE = False
    ParakeetTranscriptionWorker = None
...
class AudioToTextRecorder:
    ...
    def __init__(self,
        ...
        # Validate and store backend selection
        self.backend = backend.lower()
        if self.backend not in ["faster_whisper", "parakeet"]:
            raise ValueError(f"Unsupported backend: {backend}. Supported backends: faster_whisper, parakeet")
        
        if self.backend == "parakeet" and not PARAKEET_AVAILABLE:
            raise ImportError(
                "Parakeet backend requested but NeMo toolkit not available. "
                "Install with: pip install nemo_toolkit[asr]"
            )
        
        logger.info(f"Using transcription backend: {self.backend}")
        ...
        self.transcript_process = self._start_thread(
            ...
            args=(
                self.backend,  # Pass backend as first argument
    ...
    @staticmethod
    def _transcription_worker(backend, *args, **kwargs):
        """Create and run the appropriate transcription worker based on backend selection."""
        if backend == "faster_whisper":
            worker = TranscriptionWorker(*args, **kwargs)
        elif backend == "parakeet":
            if not PARAKEET_AVAILABLE:
                raise ImportError("Parakeet backend not available. Install with: pip install nemo_toolkit[asr]")
            worker = ParakeetTranscriptionWorker(*args, **kwargs)
        else:
